...
import { AdsFilterParamsRequestType, VacancyFilterParamsRequestType } from "../vacancy/types/FilterFormsAdsFieldsType";
...
export const FilterFormsAds = ({ handleClose, searchQuery, setPageFu }: FilterFormsAdsType) => {
  ...
  const onSubmit = (data: FormsFilterType) => {
    ...
    switch (locationTabs) {
      case routeVacancy:
        clearListVacancy();
        console.log(filterSubmit(data, "vacancy"));
    ...
    <FilterLayoutWrapper handleClose={handleClose}>
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={s.gawField}>
          <ControllersCityAsync name="city" placeholder="Город" control={control} setValue={setValue} required={false} />
          <ControllerToolsAsync control={control} placeholder="Инструмент (род деятельности)" name="tool" required={false} />
          ...
          {locationTabs === routeVacancy ? (
            <ControllerRandomSelect control={control} placeholder="Кто ищет?" name="who_is_looking_vacancy" options={optionsTypeAccount} />
          ) : locationTabs === routeAnonnsemend ? (
            <ControllerRandomSelect control={control} placeholder="Что/кого ищут?" name="who_is_looking_ads" options={requiredADS} />
          ) : (
            <ControllerRandomSelect name="who_is_looking_questionnaire" control={control} placeholder="Тип аккаунта" options={optionsTypeAccount} />
          )}

          {locationTabs === routeVacancy && (
            <>
              {who_is_looking_vacancy === EnumTypeAccount.INSTITUTION && (
                <ControllersInstitutionTypeAsyncNew name="typeOfInstitution" control={control} placeholder="Тип заведения" />
              )}
              <ControllerRandomSelect control={control} placeholder="Кого ищет?" name="who_is_looking_vacancy_partner" options={requiredVacancy} />
            </>
          )}

          {locationTabs === routeVacancy &&
            (watch_vacancy_partner === EnumTypeDocumentType.TEAM ? (
              <ControllerRandomSelect control={control} placeholder="Вид коллектива" name="teamType" options={teamTypeADS} />
            ) : (
              watch_vacancy_partner === EnumTypeDocumentType.MUSICIAN && <WatchMusician control={control} watch={watch} />
            ))}
          ...
          {locationTabs === routeAnonnsemend &&
            (watch_looking_ads === EnumTypeDocumentType.TEAM ? (
              ...
              <ControllersInstitutionTypeAsyncNew name="typeOfInstitution" control={control} placeholder="Место работы" />
            ) : null)}
